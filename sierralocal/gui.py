...
class App():
    def __init__(self, parent):
        ...
        self.runbutton = tk.Button(self.runframe, text="RUN", fg="black",
                                   command=lambda: self.runSierra(
                                       self.inputFileNameVar.get(),
                                       self.outputfileNameVar.get(),
                                       self.skipalign.get(),
                                       self.cleanup.get())
                                   )
    ...
    def runSierra(self, inputfile, outputfile, skipalign, cleanup):
        ...
        use_paste = False
        if len(pastedtext) >= 10:
            use_paste = True
            # transfer pasted sequence to temporary file stream
            handle = tempfile.TemporaryFile()
            handle.write(pastedtext)
        ...
        args = [
            'python3', 'main.py',
            handle if use_paste else inputfile,
            '-o', outputfile
        ]
        if skipalign:
            args.append('-skipalign')
        if cleanup:
            args.append('-cleanup')
        ...
        if len(pastedtext) < 10:  # no pasted sequence
            ...
            p = subprocess.Popen('python3 sierralocal.py {} -o {}{}{}'.format(
                inputfile, outputfile, skip(skipalign), clean(cleanup)
            ...
            p = subprocess.Popen('python3 sierralocal.py {} -o {}{}{}'.format(
                "temp.fa", outputfile, skip(skipalign), clean(cleanup)
            ), stdout=subprocess.PIPE, shell=True)
