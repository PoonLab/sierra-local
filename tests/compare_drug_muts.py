...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Process JSON files and extract mutations.')
    ...
    for item in data:
        ...
        for genes in item['drugResistance']:
            ...
            for drugs in genes['drugScores']:
                for scores in drugs['partialScores']:
                    if isinstance(scores, dict):
                        for mutation in scores['mutations']:
                            if mutation['text'] not in result_dict[name][gene]:
                                result_dict[name][gene].append(mutation['text'])
